...
class DecisionTree:
    ...
    def create_tree(self, classes, input_attributes, df):
        ...
        is_string = [-1]
        attribute_d = DecisionTree.get_attribute_with_the_highest_inf_gain(self, df_copy, se, input_attributes, classes,
                                                                           is_string)
        children_list = []
    ...
    def innerPredict(self, attributes, x):
        y = self.root
        while (y.children is not None) and (len(y.children) != 0):
            i = -1
            if y.is_string:
                i = 0
